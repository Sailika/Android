...
public class SlidingDrawerFragment extends ViewGroup {
	 ...
	 */
	public SlidingDrawerFragment(Context context, AttributeSet attrs) {
	 ...
	 */
	public SlidingDrawerFragment(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.MySlidingDrawer, defStyle, 0);

		int orientation = a.getInt(
				R.styleable.MySlidingDrawer_orientation,
				ORIENTATION_HORIZONTAL);
		mVertical = orientation == ORIENTATION_VERTICAL;
		mBottomOffset = (int) a.getDimension(
				R.styleable.MySlidingDrawer_bottomOffset, 0.0f);
		mTopOffset = (int) a.getDimension(
				R.styleable.MySlidingDrawer_topOffset, 0.0f);
		mAllowSingleTap = a.getBoolean(
				R.styleable.MySlidingDrawer_allowSingleTap, true);
		mAnimateOnClick = a.getBoolean(
				R.styleable.MySlidingDrawer_animateOnClick, true);

		int handleId = a.getResourceId(
				R.styleable.MySlidingDrawer_handle, 0);
		if (handleId == 0) {
		...
		int contentId = a.getResourceId(
				R.styleable.MySlidingDrawer_content, 0);
		if (contentId == 0) {
			throw new IllegalArgumentException(
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		frame.set(trackHandle.getLeft(),
				((ViewGroup) trackHandle.getParent()).getTop(),
				trackHandle.getRight(),
				((ViewGroup) trackHandle.getParent()).getBottom());

		if (!mTracking && !frame.contains((int) x, (int) y) || isOpened()) {
			return false;
		}

		if (action == MotionEvent.ACTION_DOWN && !isOpened()) {
			mTracking = true;
	...
	private View mTrackHandle;
	public void setTrackHandle(View trackHandle) {
		mTrackHandle = trackHandle;
	}
}
