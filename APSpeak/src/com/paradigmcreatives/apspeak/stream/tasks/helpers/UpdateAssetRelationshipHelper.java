...
/**
 * Helper class to make server request for asset relationship update
 * 
 * @author Dileep | neuv
 * 
 */
public class UpdateAssetRelationshipHelper {

	private static final String TAG = "UpdateAssetRelationshipHelper";
	private Context context;

	/**
	 * @param context
	 */
	public UpdateAssetRelationshipHelper(Context context) {
		...
		if (context != null && requestJSON != null) {
			...
			try {
				...
				httpPost = new HttpPost(ServerConstants.NODE_SERVER_URL
						+ ServerConstants.CREATE_ASSET_RELATIONSHIP);
